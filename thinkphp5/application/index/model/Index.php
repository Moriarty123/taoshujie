...
class index extends Controller
{
	...
	function GetNewSaleBook(){
		$sql = "select * from sale_book where sale_num!= 0 order by sale_time asc limit 0,8;";
		// $data = $this->_dao->GetRows($sql);
		$data = Db::query($sql);
		return $data;
	}
	...
	function GetNewInquiry(){
		$sql = "select * from inquiry_book where inquiry_state = 0 order by inquiry_time asc limit 0,8;";
		// $data = $this->_dao->GetRows($sql);
		$data = Db::query($sql);
		return $data;
	}
