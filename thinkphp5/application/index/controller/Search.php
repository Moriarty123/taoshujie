...
class Search extends Controller
{
    ...
    public function pacon(){

        $keyword = $_GET['keyword'];

         $rules = array(
  
           'sale_img' => array('.img-box img','src'),
           'sale_degrees' => array('.quality','html'),
           'sale_afprice' => array('.f_right .bold','html'),
           'sale_content' => array('.user-info-link span','html'),
           'sale_name' =>array('.title a','html'),
           'user_name' =>array('.zl-isbn-info span','html'),
           'img_href' => array('.item-img .img-box','href')
    );

      $html = "http://search.kongfz.com/product_result/?key=".$keyword."&status=0";
       $data = QueryList::Query($html,$rules)->data;
     
     foreach ($data as $key =>$value) {
        if(!isset($value['sale_img'])){
            unset($data[$key]);
        }
     }
            ...
            if ($lengh>5) {
               for($i=0;$i<5;$i++){
                $wantsearch[]=$data[$lengh-$i-1]['sale_name'];
                }
            }else{
                for($i=0;$i<$lengh;$i++){
                $wantsearch[]=$data[$lengh-$i-1]['sale_name'];
