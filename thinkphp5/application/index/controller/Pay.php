...
class Pay extends \think\Controller
{
    ...
    public function refund()
    {
    	require_once ROOT_PATH.'vendor/alipay/config.php';
		require_once ROOT_PATH.'/vendor/alipay/pagepay/service/AlipayTradeService.php';
		require_once ROOT_PATH.'/vendor/alipay/pagepay/buildermodel/AlipayTradeRefundContentBuilder.php';
		...
		if(!empty($response)) {
			$this->success('退款成功','/index/ordersend/orderrefund');
		}
    ...
    public function return_url(){
		...
		*/
		if($result) {//验证成功
			...
			session('trade_no', $trade_no);

			if($isbuy == true) {
				// dump('ordersend');
				$this->success('提交订单成功',"/index/ordersend/submitorder");
			}
			else{
				// dump('insertToOrder');
				$this->success('提交订单成功',"/index/shopCar/insertToOrder");
			}
		    ...
		    //验证失败
		    // echo "验证失败";
		    $this->error('提交订单失败',"/index/shopCar/index");
		}
    }
    ...
    //退款
    public function refund2($no, $trade_no, $amount, $reason, $request_no)
    {
    	require_once ROOT_PATH.'vendor/alipay/config.php';
		require_once ROOT_PATH.'/vendor/alipay/pagepay/service/AlipayTradeService.php';
		require_once ROOT_PATH.'/vendor/alipay/pagepay/buildermodel/AlipayTradeRefundContentBuilder.php';
	    ...
	    //标识一次退款请求，同一笔交易多次退款需要保证唯一，如需部分退款，则此参数必传
	    $out_request_no = trim($request_no);
